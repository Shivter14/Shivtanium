...
for /l %%# in () do (
	...
	if !timer.100cs! geq 100 (
		set /a "timer.100cs%%=100, procRange=2, procCount=0, procListH=(contentH=win[!PID!.taskmgr]H - 2) - 1"
		set "_processes=!processes!"
		set "processes="
		for /l %%a in (3 1 !procListH!) do set "pid[%%a]="
		for /f "delims=*^!" %%A in ('dir /b "!sst.dir!\temp\proc\PID-*"') do if %%~zA leq 1023 (
			set "temp.PID=%%~A"
			set "temp.PID=!temp.PID:*-=!"
			if /I "!temp.PID:~-11!" neq "-memoryDump" (
		...
		if "!_processes!" neq "!processes!" (
			set buffer=¤MW	!PID!.taskmgr	l!contentH!= Running processes: !procCount!
			for /l %%p in (3 1 !procListH!) do if defined pid[%%p] (
				for /f "usebackq" %%P in ("!sst.dir!\temp\proc\PID-!pid[%%p]!") do set "process=%%~P"
				set "buffer=!buffer!	l%%p=  !process:~0,%closeButtonX%!"
			) else set "buffer=!buffer!	l%%p="
			echo=!buffer!
			set buffer=
		)
